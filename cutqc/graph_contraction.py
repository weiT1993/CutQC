...
class GraphContractor(object):
	def __init__(self, compute_graph, subcircuit_entry_probs, num_cuts) -> None:
		...
		self.smart_order = sorted(self.subcircuit_entry_lengths.keys(),key=lambda subcircuit_idx:self.subcircuit_entry_lengths[subcircuit_idx])
	...
	def compute(self):
		...
		dataset = dataset.batch(batch_size=1,num_parallel_calls=tf.data.AUTOTUNE,deterministic=False)
		self.times['make_dataset'] = perf_counter() - make_dataset_begin
		
		compute_begin = perf_counter()
		dataset = dataset.map(compute_summation_term,num_parallel_calls=tf.data.AUTOTUNE,deterministic=False)
