...
def get_dirname(circuit_name,max_subcircuit_width,eval_mode,num_threads,mem_limit,field):
    '''
    Directory management for CutQC.
    '''
    if field=='cutter':
        dirname = './cutqc_data/%s/qc_%d'%(circuit_name,max_subcircuit_width)
    elif field=='evaluator':
        dirname = './cutqc_data/%s/qc_%d/%s'%(circuit_name,max_subcircuit_width,eval_mode)
    elif field=='build':
        dirname = './cutqc_data/%s/qc_%d/%s_%d_%d'%(circuit_name,max_subcircuit_width,eval_mode,mem_limit,num_threads)
    elif field=='slurm':
        dirname = './slurm/%s/qc_%d'%(circuit_name,max_subcircuit_width)
    elif field=='runtime':
        dirname = './runtime/%s/qc_%d/q_%d_%d'%(circuit_name,max_subcircuit_width,mem_limit,num_threads)
    else:
