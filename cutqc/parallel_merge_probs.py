...
if __name__ == '__main__':
    ...
    for subcircuit_idx in meta_info['entry_init_meas_ids']:
        rank_jobs = find_process_jobs(jobs=list(meta_info['entry_init_meas_ids'][subcircuit_idx].keys()),
        rank=args.rank,num_workers=args.num_workers)
        ...
        for subcircuit_entry_init_meas in rank_jobs:
            ...
            unmerged_prob_vector = pickle.load(open('%s/subcircuit_%d_entry_%d.pckl'%(args.data_folder,subcircuit_idx,subcircuit_entry_id),'rb'))
            merged_subcircuit_entry_probs[subcircuit_idx][subcircuit_entry_init_meas] = merge_prob_vector(unmerged_prob_vector=unmerged_prob_vector,
            qubit_states=dd_schedule['subcircuit_state'][subcircuit_idx])
    pickle.dump(merged_subcircuit_entry_probs,open('%s/rank_%d_merged_entries.pckl'%(args.data_folder,args.rank),'wb'))