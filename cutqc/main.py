...
class CutQC:
    ...
    def build(self, mem_limit, recursion_depth):
        ...
        dd = DynamicDefinition(compute_graph=self.compute_graph,
        data_folder=self.tmp_data_folder,num_cuts=self.num_cuts,
        mem_limit=mem_limit,recursion_depth=recursion_depth)
        dd.build()
    ...
    def verify(self):
        ...
        reconstructed_prob, self.approximation_error = full_verify(full_circuit=self.circuit,
    ...
    def _generate_metadata(self):
        self.compute_graph = generate_compute_graph(counter=self.counter,
        subcircuits=self.subcircuits,
        complete_path_map=self.complete_path_map)
        ...
        if self.verbose:
            ...
            for subcircuit_idx in self.subcircuit_entries:
                print('Subcircuit_%d has %d entries'%(subcircuit_idx,len(self.subcircuit_entries[subcircuit_idx])))
    ...
    def _run_subcircuits(self):
        ...
        run_subcircuit_instances(
            ...
            eval_mode=self.eval_mode,num_shots_fn=self.num_shots_fn,
    ...
    def _attribute_shots(self):
        ...
        attribute_shots(subcircuit_entries=self.subcircuit_entries,
        subcircuits=self.subcircuits,
        eval_mode=self.eval_mode,
        data_folder=self.tmp_data_folder)
        subprocess.call('rm %s/subcircuit*instance*.pckl'%self.tmp_data_folder,shell=True)