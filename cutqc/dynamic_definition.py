...
class DynamicDefinition(object):
    def __init__(self, compute_graph, data_folder, num_cuts, mem_limit, recursion_depth) -> None:
        super().__init__()
    ...
    def build(self):
		...
		'''
        num_qubits = sum([self.compute_graph.nodes[subcircuit_idx]['effective'] for subcircuit_idx in self.compute_graph.nodes])
        ...
        while recursion_layer<self.recursion_depth:
            ...
            pickle.dump(dd_schedule,open('%s/dd_schedule.pckl'%self.data_folder,'wb'))
            ...
            graph_contractor = GraphContractor(compute_graph=self.compute_graph,
            ...
            if recursion_layer<self.recursion_depth-1 and has_merged_states:
                bin_indices = np.argpartition(reconstructed_prob, -self.recursion_depth)[-self.recursion_depth:]
                ...
                largest_bins = sorted(largest_bins,key=lambda bin:bin['prob'],reverse=True)[:self.recursion_depth]
    ...
    def initialize_dynamic_definition_schedule(self):
        ...
        subcircuit_capacities = {subcircuit_idx:self.compute_graph.nodes[subcircuit_idx]['effective'] for subcircuit_idx in self.compute_graph.nodes}
        subcircuit_active_qubits = self.distribute_load(capacities=subcircuit_capacities)
        ...
        for subcircuit_idx in subcircuit_active_qubits:
            ...
            num_merged = self.compute_graph.nodes[subcircuit_idx]['effective'] - num_zoomed - num_active
    ...
    def next_dynamic_definition_schedule(self, recursion_layer, bin_id):
        ...
        for subcircuit_idx in self.dd_bins[recursion_layer]['subcircuit_state']:
            num_active += self.dd_bins[recursion_layer]['subcircuit_state'][subcircuit_idx].count('active')
        ...
        next_dd_schedule = {'subcircuit_state':copy.deepcopy(self.dd_bins[recursion_layer]['subcircuit_state'])}
    ...
    def merge_states_into_bins(self):
        ...
        num_entries = [len(meta_info['entry_init_meas_ids'][subcircuit_idx]) for subcircuit_idx in self.compute_graph.nodes]
        subcircuit_num_qubits = [self.compute_graph.nodes[subcircuit_idx]['effective'] for subcircuit_idx in self.compute_graph.nodes]
        num_workers = get_num_workers(num_jobs=max(num_entries),ram_required_per_worker=2**max(subcircuit_num_qubits)*4/1e9)
        procs = []
        for rank in range(num_workers):
            python_command = 'python -m cutqc.parallel_merge_probs --data_folder %s --rank %d --num_workers %d'%(
        ...
        for rank in range(num_workers):
            rank_merged_subcircuit_entry_probs = pickle.load(open('%s/rank_%d_merged_entries.pckl'%(self.data_folder,rank),'rb'))
            ...
            subprocess.run(['rm','%s/rank_%d_merged_entries.pckl'%(self.data_folder,rank)])
...
def full_verify(full_circuit, complete_path_map, subcircuits, dd_bins):
    ...
    subcircuit_out_qubits = get_reconstruction_qubit_order(full_circuit=full_circuit, complete_path_map=complete_path_map, subcircuits=subcircuits)
    reconstructed_prob = read_dd_bins(subcircuit_out_qubits=subcircuit_out_qubits,dd_bins=dd_bins)
    real_probability = quasi_to_real(quasiprobability=reconstructed_prob,mode='nearest')
    approximation_error = MSE(target=ground_truth,obs=real_probability)*2**full_circuit.num_qubits/np.linalg.norm(ground_truth)**2
