...
class ComputeGraph(object):
	...
	def get_init_meas(self, subcircuit_idx):
		...
		for edge in edges_to_node:
			...
			entry_init[bare_subcircuit.qubits.index(edge_attributes['rho_qubit'])] = edge_attributes['basis']
		...
		for edge in edges_from_node:
			...
			entry_meas[bare_subcircuit.qubits.index(edge_attributes['O_qubit'])] = edge_attributes['basis']
		return (tuple(entry_init), tuple(entry_meas))
	
	def get_contraction_edges(self, leading_subcircuit_idx, contracted_subcircuits_indices):
		'''
		...
		for edge in self.edges:
			u_for_edge, v_for_edge, _ = edge
			if u_for_edge in contracted_subcircuits_indices and v_for_edge in contracted_subcircuits_indices:
				continue
			elif u_for_edge==leading_subcircuit_idx and v_for_edge in contracted_subcircuits_indices:
				contraction_edges.append(edge)
			elif v_for_edge==leading_subcircuit_idx and u_for_edge in contracted_subcircuits_indices:
				contraction_edges.append(edge)
	...
	def get_leading_edges(self, leading_subcircuit_idx, contracted_subcircuits_indices):
		...
		for edge in self.edges:
			...
			if u_for_edge in contracted_subcircuits_indices and v_for_edge in contracted_subcircuits_indices:
				continue
			elif u_for_edge==leading_subcircuit_idx and v_for_edge not in contracted_subcircuits_indices:
				leading_edges.append(edge)
			elif v_for_edge==leading_subcircuit_idx and u_for_edge not in contracted_subcircuits_indices:
				leading_edges.append(edge)
		return leading_edges
	
	def get_trailing_edges(self, leading_subcircuit_idx, contracted_subcircuits_indices):
		'''
		...
		for edge in self.edges:
			...
			if u_for_edge in contracted_subcircuits_indices and v_for_edge in contracted_subcircuits_indices:
				continue
			elif u_for_edge in contracted_subcircuits_indices and v_for_edge!=leading_subcircuit_idx:
				trailing_edges.append(edge)
			elif v_for_edge in contracted_subcircuits_indices and u_for_edge!=leading_subcircuit_idx:
				trailing_edges.append(edge)
...
def generate_compute_graph(counter, subcircuits, complete_path_map):
	...
	for subcircuit_idx in counter:
		...
		compute_graph.add_node(subcircuit_idx=subcircuit_idx,attributes=subcircuit_attributes)
	for circuit_qubit in complete_path_map:
		...
		for counter in range(len(path)-1):
			...
			compute_graph.add_edge(u_for_edge=upstream_subcircuit_idx,v_for_edge=downstream_subcircuit_idx,
			attributes={'O_qubit':path[counter]['subcircuit_qubit'],'rho_qubit':path[counter+1]['subcircuit_qubit']})
...
def generate_subcircuit_entries(compute_graph):
	...
	for subcircuit_idx in compute_graph.nodes:
		...
		for subcircuit_edge_bases in itertools.product(['I','X','Y','Z'],repeat=len(subcircuit_edges)):
			...
			for edge_basis, edge in zip(subcircuit_edge_bases, subcircuit_edges):
				upstream_subcircuit_idx, downstream_subcircuit_idx, edge_attributes = edge
				if subcircuit_idx==upstream_subcircuit_idx:
					O_qubit = edge_attributes['O_qubit']
					subcircuit_entry_meas[bare_subcircuit.qubits.index(O_qubit)] = edge_basis
				elif subcircuit_idx==downstream_subcircuit_idx:
					rho_qubit = edge_attributes['rho_qubit']
					subcircuit_entry_init[bare_subcircuit.qubits.index(rho_qubit)] = edge_basis
				else:
					raise IndexError('Generating entries for a subcircuit. subcircuit_idx should be either upstream or downstream')
			# print('subcircuit_entry_init =',subcircuit_entry_init)
			# print('subcircuit_entry_meas =',subcircuit_entry_meas)
			subcircuit_instance_init_meas = get_instance_init_meas(init_label=subcircuit_entry_init,
			...
			for init_meas in subcircuit_instance_init_meas:
				...
				coefficient, instance_init = convert_to_physical_init(init=list(instance_init))
				if (instance_init,instance_meas) not in subcircuit_instances[subcircuit_idx]:
					subcircuit_instances[subcircuit_idx].append((instance_init,instance_meas))
				subcircuit_entry_term.append((coefficient, (instance_init, instance_meas)))
				# print('%d *'%coefficient, instance_init, instance_meas)
			subcircuit_entries[subcircuit_idx][(tuple(subcircuit_entry_init),tuple(subcircuit_entry_meas))] = subcircuit_entry_term
